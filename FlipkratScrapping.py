...
from cassandra_op import CassandraManagement
                        ...
                        while count <= total_review_page:
                            ...
                            if len(ratings) > 0:
                                for i in range(0, len(ratings)):
                                    ...
                                    result_cass = (product_name, product_searched, price, offer_details, discount_percent, EMI, ratings[i], comment[i], customer_name[i], review_age[i])
                                    mongoClient.insertRecord(db_name="Flipkart-Scrapper",
                                                             collection_name=searchString,
                                                             record=result)
                                    cassandraClient.create_connection(record=result_cass)
                                    print(result)
